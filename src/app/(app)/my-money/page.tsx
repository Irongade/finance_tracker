...
export default function MyMoneyPage() {
  ...
  return (
    <>
      ...
      {/* two independent stacks, not grid rows: a short card never inherits a tall neighbour's row height */}
        ...
        <div className="grid gap-4">
          ...
          >
            {investments.length === 0 ? (
              <p className="text-[13px] text-ink-muted">No investment accounts yet.</p>
            ) : null}
            {investments.map((a) => (
              <LineItem
                key={a.account.id}
          ...
          <SectionCard title="Monthly snapshot" description="The leftover waterfall">
            ...
            />
            <LineItem
              label="less: goal contributions"
              value={<MoneyText pence={-person.pledgesPence} style="whole" />}
            />
            <LineItem
              label="less: debt payments"
              value={<MoneyText pence={-person.debtPaymentsPence} style="whole" />}
            />
            <LineItem
